...
import { getActivePoll, submitVote } from '../services/votingService';
...
const VotingPage: React.FC<VotingPageProps> = ({ currentUser }) => {
  ...
  const HAS_VOTED_KEY = `user-has-voted-${currentUser}`;

  useEffect(() => {
    const checkVotedStatus = () => {
      const voted = localStorage.getItem(HAS_VOTED_KEY);
      if (voted === 'true') {
        setHasVoted(true);
      }
    };
    ...
    return (
      <div className="relative text-center p-8 mt-10 min-h-[calc(100vh-4rem)] flex flex-col justify-center items-center overflow-hidden">
        ...
        <div className="z-10">
          ...
          <p className="mt-2 text-lg text-gray-300">Your vote has been counted. {currentUser === 'admin' && 'Check the dashboard to see live results.'}</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="relative text-center p-8 mt-10 min-h-[calc(100vh-4rem)] flex flex-col justify-center items-center overflow-hidden">
        ...
        <div className="z-10">
          ...
          <p className="mt-2 text-lg text-gray-300">Please wait for an admin to create a poll.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="relative min-h-[calc(100vh-4rem)] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 z-10 relative">
        ...
        {filteredCandidates.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 md:gap-6 pb-28">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gray-900/80 backdrop-blur-sm border-t border-gray-700 z-10">
        <div className="max-w-xl mx-auto">
          <button
            ...
            className="w-full bg-red-600 text-white font-bold py-4 px-4 rounded-lg shadow-lg hover:bg-red-700 disabled:bg-gray-500 disabled:cursor-not-allowed transition-all duration-300 text-lg flex items-center justify-center"
          >
            {isSubmitting ? (
              <>
